...
import Vista.Vista_ventas_credito;
 ...
 * @author Luciano Ces
 */
public class ControladorVentasCredito implements Interface_agregar_cliente, Interface_main_functions {
    ...
    private Vista_ventas_credito vista_credito;
    private Vista_ventas_detalle vista_detalle;
    private Vista_ventas_editar vista_editar;
    private DaoResumen daoResumen;
    private ControladorEmpleado controladorEmpleado;

    public ControladorVentasCredito() {

    }

    @Override
    public void iniciar() {
        this.vista_credito = new Vista_ventas_credito(this);
        this.daoResumen = new DaoResumen();
        FUNCIONES.abrir_ventana(vista_credito);
        this.fila_ventas = -1;
        iniciar_dateChooser();
        mostrar("");
    }
    ...
    public void mostrar(String texto) {
        try {
            ...
            modelo = daoResumen.mostrar(texto, "credito", fecha_in(), fecha_out());
            vista_credito.tablaVentas.setModel(modelo);
            total_resumen();
    ...
    private void iniciar_dateChooser() {
        Common.configurar_calendario(vista_credito.fechaInicial);
        Common.configurar_calendario(vista_credito.fechaFinal);
        java.util.Date date;
        try {
            date = new SimpleDateFormat("d-M-yyyy").parse("01-01-2018");
            vista_credito.fechaInicial.setDate(date);
        } catch (ParseException ex) {
            Logger.getLogger(ControladorVentasCredito.class.getName()).log(Level.SEVERE, null, ex);
        }

        vista_credito.fechaInicial.getDateEditor().addPropertyChangeListener(new PropertyChangeListener() {
            @Override
        ...
        vista_credito.fechaFinal.getDateEditor().addPropertyChangeListener(new PropertyChangeListener() {
            @Override
            public void propertyChange(PropertyChangeEvent e) {
                if (e.getPropertyName().equals("date")) {
                    mostrar("");
    ...
    private Date fecha_in() {
        Date fecha_in = new Date(vista_credito.fechaInicial.getDate().getTime());
        return fecha_in;
    }

    private Date fecha_out() {
        Date fecha_out = new Date(vista_credito.fechaFinal.getDate().getTime());
        return fecha_out;
    }

    protected void ocultar_columnas_ventas() {
        Integer[] cols = {4, 6, 7};
        FUNCIONES.ocultar_columnas(cols, vista_credito.tablaVentas);
        vista_credito.tablaVentas.getColumnModel().getColumn(1).setPreferredWidth(150);
        vista_credito.tablaVentas.getColumnModel().getColumn(2).setPreferredWidth(300);
    }

    protected void total_resumen() {
        double total_soles = 0.0;
        for (int i = 0; i < vista_credito.tablaVentas.getRowCount(); i++) {
            total_soles += Double.parseDouble(vista_credito.tablaVentas.getValueAt(i, 3).toString());
        }
        vista_credito.lblTotal_soles.setText(Common.aDecimal(total_soles));
    }
    ...
    public void seleccionar_item() {
        fila_ventas = vista_credito.tablaVentas.getSelectedRow();
        if (fila_ventas == -1) {
            idventa = -1;
        } else {
            idventa = Integer.parseInt(vista_credito.tablaVentas.getValueAt(fila_ventas, 0).toString());
        }
    }
    ...
    public void editar_item() {
        ...
        if (fila_ventas == -1) {
            ...
            try {
                ...
                vista_editar = new Vista_ventas_editar(f, true, null, this);
                ...
                idcliente = vista_credito.tablaVentas.getValueAt(fila_ventas, 6) != null ? Integer.parseInt(vista_credito.tablaVentas.getValueAt(fila_ventas, 6).toString()) : -1;
                mostrar_empleados();
                Common.configurar_calendario(vista_editar.fechaInicial);

                String fc = vista_credito.tablaVentas.getValueAt(fila_ventas, 1).toString();
...
//Diálogo ver detalle
    public void abrir_dialogo_detalle() {
        ...
        if (fila_ventas == -1) {
            ...
            try {
                Frame f = JOptionPane.getFrameForComponent(null);
                vista_detalle = new Vista_ventas_detalle(f, true, null, this);
    ...
    private String obtener_dato_de_tabla(int col) {
        return vista_credito.tablaVentas.getValueAt(fila_ventas, col) != null ? vista_credito.tablaVentas.getValueAt(fila_ventas, col).toString() : "";
    }

    public void calcular_total(int fila) {
        double total = Double.parseDouble(vista_credito.tablaVentas.getValueAt(fila, 3).toString());
        double subtotal;
        double igv;
        subtotal = total / 1.18;
        igv = total - subtotal;
    ...
    public void dialogo_cambiar_direccion() {
        Frame f = JOptionPane.getFrameForComponent(null);
        Vista_ventas_direccion vista_direccion = new Vista_ventas_direccion(f, true, null, this);

        FUNCIONES.abrir_dialogo(vista_direccion);
    }

    private void ocultar_columnas_detalle() {
        vista_detalle.tablaDetalle.getColumnModel().getColumn(0).setPreferredWidth(250);
    }
